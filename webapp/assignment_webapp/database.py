...
def check_login(username, password):
    ...
    try:
        ...
        sql = """
         SELECT username, password FROM mediaserver.useraccount WHERE username = %s and password = %s;
        """
        print(username)
        print(password)
...
def user_playlists(username):
    ...
    try:
        ...
        ###############################################################################
        sql = """
        SELECT collection_id, collection_name FROM mediaserver.useraccount JOIN
        mediaserver.mediacollection USING(username) WHERE username = %s;
        """
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        sql = """
        SELECT podcast_id, podcast_title, podcast_uri, podcast_last_updated FROM (mediaserver.useraccount JOIN
        mediaserver.subscribed_podcasts USING(username)) JOIN mediaserver.podcast
        USING(podcast_id) WHERE username = %s;
        """
...
def user_in_progress_items(username):
    ...
    try:
        ...
        sql = """
        SELECT media_id, play_count, progress, lastviewed, storage_location FROM (mediaserver.useraccount JOIN
        mediaserver.usermediaconsumption USING(username)) JOIN mediaserver.mediaitem USING(media_id) WHERE
        progress <> 100 AND username = %s;
...
def get_song_AblumArtwork(song_id):
    """
    Get the ablum's artwork for a song by their ID
    """
    conn = database_connect()
    if(conn is None):
        return None
    ...
    try:
        sql = """
        SELECT song_id, md_type_id, md_id,md_value,md_type_name
        FROM mediaserver.Song s NATURAL JOIN mediaserver.album_songs
            LEFT JOIN (mediaserver.albummetadata NATURAL JOIN mediaserver.metadata NATURAL JOIN mediaserver.metadatatype) using (album_id)
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
            SELECT podcast_id, podcast_title, podcast_uri,
            podcast_last_updated, md_type_name, md_value FROM ((mediaserver.podcast JOIN mediaserver.podcastmetadata
            USING(podcast_id)) JOIN mediaserver.metadata USING(md_id))
            JOIN mediaserver.metadatatype USING(md_type_id) WHERE podcast_id = %s;
        """
...
def get_all_podcasteps_for_podcast(podcast_id):
    ...
    try:
        ...
        sql = """
            SELECT * FROM mediaserver.podcastepisode
            WHERE podcast_id = %s
...
def get_album(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT a.album_title, md_value, md_type_name
        FROM mediaserver.Album a LEFT JOIN
            (mediaserver.albummetadata NATURAL JOIN mediaserver.metadata NATURAL JOIN mediaserver.metadatatype) b
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT song.song_id, song.song_title, array_to_string(array_agg(distinct artist_name), ',') AS artists
        FROM mediaserver.album_songs
        JOIN mediaserver.song ON mediaserver.album_songs.song_id=mediaserver.song.song_id
        JOIN mediaserver.song_artists ON mediaserver.song.song_id=mediaserver.song_artists.song_id
        JOIN mediaserver.artist ON mediaserver.song_artists.performing_artist_id=mediaserver.artist.artist_id
        WHERE album_id = %s
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
             SELECT album_id, album_title, song_title, md_value  FROM ((((mediaserver.album JOIN mediaserver.album_songs
             USING (album_id)) JOIN mediaserver.song USING(song_id))
             JOIN mediaserver.audiomedia ON(media_id = song_id))
             JOIN mediaserver.mediaitemmetadata USING(media_id))
             JOIN mediaserver.metadata USING(md_id) WHERE album_id = %s;
        """
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT t.tvshow_title, md_value, md_type_name
        FROM mediaserver.tvshow t LEFT JOIN
                (mediaserver.tvshowmetadata NATURAL JOIN mediaserver.metadata NATURAL JOIN mediaserver.metadatatype) b
                ON (t.tvshow_id = b.tvshow_id)
        WHERE t.tvshow_id = %s
        """
...
def get_all_tvshoweps_for_tvshow(tvshow_id):
    ...
    try:
        ...
        JOIN mediaserver.tvshow ON mediaserver.tvepisode.tvshow_id = mediaserver.tvshow.tvshow_id
        WHERE tvepisode.tvshow_id = %s
        ORDER BY season ASC, episode ASC, tvepisode.air_date ASC
        """

        #GROUP BY mediaserver.tvshow.tvshow_id, season, episode, tvepisode.air_date
...
def find_matchingartists(searchterm):
    ...
    try:
        ...
        FROM mediaserver.artist m,
            (SELECT
                m1.artist_id, COUNT(me1.md_id) AS count
            FROM
                mediaserver.artist m1 LEFT OUTER JOIN mediaserver.artistmetadata me1 ON (m1.artist_id=me1.artist_id)
            GROUP BY m1.artist_id) mnew
        WHERE m.artist_id = mnew.artist_id AND LOWER(artist_name) ~ LOWER(%s)
        ORDER BY m.artist_id;
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting All artist:", sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
    ...
    try:	
        ...
        sql = """	
			...
			left outer join mediaserver.artist ar1 on (sa1.performing_artist_id=ar1.artist_id)	
                group by a1.album_id) anew	
            where a.album_id = anew.album_id	
          AND LOWER(a.album_title) LIKE LOWER (%s);"""	
...
def find_matchingsongs(searchterm):	
    ...
    try:	
        ...
        select	
            ...
            WHERE LOWER(s.song_title) LIKE LOWER(%s)	
            group by s.song_id, s.song_title	
            order by s.song_id;	
        """	
        ...
        sql = """select	
            ...
            from	
                ...
                from	
                    ...
                    group by p1.podcast_id) pnew	
            where p.podcast_id = pnew.podcast_id	
            AND LOWER(p.podcast_title) LIKE LOWER(%s);	
        """	
...
def add_SongArtist_to_db(song_id, artist_id):
    conn = database_connect()
    if(conn is None):
        return None
    cur = conn.cursor()

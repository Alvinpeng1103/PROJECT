...
/*I really made an effort on this assignment, spent almost two weeks only on it, which makes me 

have little time to do assignments another unit .  Maybe my skill is not good

enough but I am still keep learning. I found that the debugging is really annoying and I spent most of time on it

I want to know if there will be additional resources for teaching debugging skills. Also, I know it may be a little bit

harder but I still want to know if there will be feedback on our solution (not just asking question) in our interview
...
/*sorry I tried to use head file here but I failed*/

/*I really made an effort on this assignment */
...
void free_all(struct Func **list){
...
int read_file(char **argv, struct Func **func_list){
    FILE *fp;
...
int main(int argc, char **argv){
    ...
    struct Val *ram = &RAM[0];
    
    struct Reg Register[8];
    
    struct Reg *r = &Register[0];

    //read data, then return head of function list
    
    struct Func **func_list = malloc(sizeof(struct Func*) * 8);
    ...
    while(r[7].value != func_list[0]->info.nums - 1){
        
        
        if(execute(r, ram, func_list) == 1){
            printf("error\n");
            free_all(func_list);
            func_list = NULL;
            return 1;
